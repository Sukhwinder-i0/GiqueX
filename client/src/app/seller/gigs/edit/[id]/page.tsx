...
export default function EditGigPage() {
  ...
  const fetchGig = async () => {
    try {
      ...
      const gig = res.data?.data || res.data;
      setFormData({
        title: gig.title || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/gigs/update/${params.id}`, data, {
        headers: {
  ...
  return (
    <div className="min-h-screen text-white py-20 px-6">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold mb-8">Edit Gig</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">New Images (optional, up to 5)</label>
            <input
              type="file"
              multiple
              accept="image/*"
          ...
          <div className="flex gap-4">
            <Button
              type="submit"
              text={loading ? "Updating..." : "Update Gig"}
              variant="primary"
              disabled={loading}
              className="flex-1"
            />
